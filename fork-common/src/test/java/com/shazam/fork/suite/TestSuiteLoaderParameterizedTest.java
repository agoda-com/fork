...
public class TestSuiteLoaderParameterizedTest {
    ...
    public void setUp() throws Exception {
        testSuiteLoader = new TestSuiteLoader(ANY_INSTRUMENTATION_APK_FILE, fakeDexFileExtractor, fakeTestClassMatcher,
                "com.shazam.annotations.CustomParameterizedAnnotation",
                "",new TestCaseEventFactory(new TestStatsLoader(new StatServiceLoader(""))));
    }

    @Test
    public void testParameterizedTests() throws Exception {
        Collection<TestCaseEvent> events = testSuiteLoader.loadTestSuite();
        assertThat(events.size(),is(1));
        assertThat(events, hasItem(sameTestEventAs(null, "com.shazam.forktest.ParameterizedTests", false)));
    }
}
